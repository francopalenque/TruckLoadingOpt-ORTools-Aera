...
from src.common.constants import order_in_original_truck_slack, order_slack_flag, stage_one_truck_count_not_within_range, stage_one_truck_unused_cost, order_in_original_truck_constraint
import math


class Constraints:
    def __init__(self, model, model_variables, input_data):
        self.model = model
    ...
    def create_constraints(self):
        ...
        if order_in_original_truck_constraint:
    ...
    def create_only_one_time_selection_constraint(self):
        ...
        for key, po_list in self.input_data.order_truck_mapping.items():
            ...
            for (po, period) in po_list:
                expr.add(self.model_vars.order_allocation_var[(sales_document, sales_document_item, schedule_line, material, po_number, po, period)], 1)
            name_key = [str(x) for x in key]
            ct_name = "Order_Only_One_Time_Selection" + "|" + '|'.join(name_key)
            slk = 0
            if order_slack_flag:
                if key in self.model_vars.order_non_selection_var:
                    slk = self.model_vars.order_non_selection_var[key]
    ...
    def create_weight_capacity_constraints(self):
        expr = LinExpr()
        for (po, period), order_list in self.input_data.truck_selection_combinations.items():
            ...
            for key in order_list:
                weight = round(self.input_data.order_allocation_combinations[key]['gross_weight'], 6)
    ...
    def create_volume_capacity_constraints(self):
        ...
        for (po, period), order_list in self.input_data.truck_selection_combinations.items():
            ...
            for key in order_list:
                volume = round(self.input_data.order_allocation_combinations[key]['volume'], 6)
    ...
    def create_pallet_capacity_constraints(self):
        ...
        for (po, period), order_list in self.input_data.truck_selection_combinations.items():
            ...
            for key in order_list:
                pallet = round(self.input_data.order_allocation_combinations[key]['pallet_spot'], 6)
    ...
    def create_truck_period_limitation_constraints(self):
        expr = LinExpr()
        for period, po_list in self.input_data.period_truck_mapping.items():
            for po in po_list:
            ...
            if self.input_data.period_day_name_mapping[period]['day_name'] in self.input_data.dc_slot_schedule_dict:
                avail_slots = self.input_data.dc_slot_schedule_dict[self.input_data.period_day_name_mapping[period]['day_name']]
            else:
                avail_slots = 0
    ...
    def create_truck_only_one_time_selection_over_period_constraints(self):
        expr = LinExpr()
        for po, period_list in self.input_data.truck_period_mapping.items():
    ...
    def create_order_schedule_line_mapping_constraints(self):
        ...
        for key, po_details in self.input_data.order_selection_mapping_dict.items():
            ...
            for (po, period), schedule_line_lst in po_details.items():
                ...
                for schedule_line in schedule_line_lst:
                    order_allocation_var = self.model_vars.order_allocation_var[(sales_document, sales_document_item, schedule_line, material, po_number, po, period)]
                ...
                self.model.addConstr(expr_mapping <= order_item_sel_var * len(schedule_line_lst), name=ct_name)
                expr_mapping.clear()

            name_key = [str(x) for x in key]
            ct_name = "Order_Selection" + "|" + '|'.join(name_key)
            self.model.addConstr(expr_selection <= self.input_data.model_parameters['max_trucks_per_order_item'] + self.model_vars.order_item_selection_slack_var[key], name=ct_name)
    ...
    def create_shuffle_together_balancing_constraints(self):
        ...
        for key, schedule_line_lst in self.input_data.shuffle_order_schedule_line_mapping_dict.items():
            sales_document, sales_document_item, material, po_number = key
            ...
            for (po, period) in self.input_data.shuffle_together_mapping_dict[key]:
                if schedule_line_lst == self.input_data.shuffle_together_mapping_dict[key][(po, period)]:
                    for sch_lne in list(schedule_line_lst)[1:]:
                        anchor_order_key = (sales_document, sales_document_item, anchor_schedule_line, material, po_number, po, period)
                        mapping_order_key = (sales_document, sales_document_item, sch_lne, material, po_number, po, period)
                    ...
                    for sch_lne in self.input_data.shuffle_together_mapping_dict[key][(po, period)]:
                        self.model_vars.order_allocation_var[(sales_document, sales_document_item, sch_lne, material, po_number, po, period)].ub = 0
    ...
    def create_total_truck_selection_preference_cost_objective(self):
        ...
        for key in self.model_vars.truck_selection_var.keys():
            po, period = key
            cost = self.input_data.truck_po_level_details[po]['under_utilization_cost'] if po in self.input_data.truck_po_level_details else 100
            if math.isnan(cost):
                cost = 100
    ...
    def create_total_truck_period_delay_cost_objective(self):
        expr = LinExpr()
        for po, values in self.input_data.truck_po_level_details.items():
            requested_delivery_period = values['requested_delivery_period']
            if po in self.input_data.truck_period_mapping:
                for period in self.input_data.truck_period_mapping[po]:
    ...
    def create_total_order_selection_cost_objective(self):
        ...
        for key in self.model_vars.order_non_selection_var.keys():
            sales_document, sales_document_item, schedule_line, material, po_number = key
            cost = 1
            if po_number in self.input_data.apo_truck_load_dict and (sales_document, sales_document_item, schedule_line, material) in self.input_data.apo_truck_load_dict[po_number]:
                if self.input_data.apo_truck_load_dict[po_number][(sales_document, sales_document_item, schedule_line, material)]['priority_line_flag'] == 'Y':
                    cost = 100
...
class ReshufflingStageOneConstraints:
    def __init__(self,model,model_variables,input_data,updated_order_allocation_combinations, updated_order_truck_mapping, updated_truck_selection_combinations,
    ...
    def create_only_one_time_selection_constraint(self):
        expr = LinExpr()
        for key, po_list in self.updated_order_truck_mapping.items():
            sales_document, sales_document_item, schedule_line, material, po_number = key
            for (po, period) in po_list:
                expr.add(self.model_vars.order_allocation_var[(sales_document, sales_document_item, schedule_line, material, po_number, po, period)], 1)
            name_key = [str(x) for x in key]
    ...
    def create_weight_capacity_constraints(self):
        ...
        for (po, period), order_list in self.updated_truck_selection_combinations.items():
            ...
            weight_limit = self.input_data.truck_po_level_details[po]['weight_constraint'] if po in self.input_data.truck_po_level_details else 0
            for key in order_list:
                weight = round(self.updated_order_allocation_combinations[key]['gross_weight'], 6)
                expr.add(self.model_vars.order_allocation_var[key], weight)
    ...
    def create_volume_capacity_constraints(self):
        ...
        for (po, period), order_list in self.updated_truck_selection_combinations.items():
            ...
            volume_limit = self.input_data.truck_po_level_details[po]['volume_constraint'] if po in self.input_data.truck_po_level_details else 0
            for key in order_list:
                volume = round(self.updated_order_allocation_combinations[key]['volume'], 6)
                expr.add(self.model_vars.order_allocation_var[key], volume)
    ...
    def create_pallet_capacity_constraints(self):
        ...
        for (po, period), order_list in self.updated_truck_selection_combinations.items():
            ...
            pallet_limit = self.input_data.truck_po_level_details[po]['pallet_constraint'] if po in self.input_data.truck_po_level_details else 0
            for key in order_list:
                pallet = round(self.updated_order_allocation_combinations[key]['pallet_spot'], 6)
                expr.add(self.model_vars.order_allocation_var[key], pallet)
        ...
        for key, schedule_line_lst in self.updated_shuffle_order_schedule_line_mapping_dict.items():
            ...
            for (po, period) in self.updated_shuffle_together_mapping_dict[key]:
                if schedule_line_lst == self.updated_shuffle_together_mapping_dict[key][(po, period)]:
                    ...
                    for sch_lne in self.updated_shuffle_together_mapping_dict[key][(po, period)]:
                        self.model_vars.order_allocation_var[(sales_document, sales_document_item, sch_lne, material, po_number, po, period)].ub = 0

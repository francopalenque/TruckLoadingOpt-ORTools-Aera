import time
from src.stage_one.model_building.decision_variables import ReshufflingStageOneDecisionVariable
from src.stage_one.model_building.constraints import ReshufflingStageOneConstraints
...
def process_previous_output(raw_output_dict):
    ...
    for key, values in raw_output_dict['order_allocation_dict'].items():
        ...
        if values >= 1:
            order_sel_lst.add((sales_document, sales_document_item, schedule_line, material, po_number))
...
def process_data_preprocessing(input_data, order_sel_lst, sel_po_period_lst, sel_period_lst):
    updated_order_allocation_combinations = {}
    for order_key in order_sel_lst:
        sales_document, sales_document_item, schedule_line, material, po_number = order_key
        ord_details = input_data.apo_truck_load_dict[po_number][(sales_document, sales_document_item, schedule_line, material)]
        if ord_details['priority_line_flag'] == 'Y':
            if ord_details['requested_delivery_period'] <= ord_details['delivery_period']:
                period_lst = []
                result = find_valid_period(ord_details['delivery_period'], input_data.horizon, input_data.period_day_name_mapping, input_data.dc_slot_schedule_dict)
                ...
                for period in range(ord_details['delivery_period'], ord_details['requested_delivery_period'] + 1):
                    ...
                    if input_data.dc_slot_schedule_dict.get(day_name, 0) > 0:
                        period_lst.append(period)

                # If nothing found, fallback after requested
                if not period_lst:
                    result = find_valid_period(ord_details['requested_delivery_period'], input_data.horizon, input_data.period_day_name_mapping, input_data.dc_slot_schedule_dict)
                    if result is not None:
                        period_lst.append(result)
        else:
        ...
        for (po, period) in sel_po_period_lst:
            if period >= ord_details['delivery_period'] and period in period_lst:
                updated_order_allocation_combinations[(sales_document, sales_document_item, schedule_line, material, po_number, po, period)] = ord_details
    ...
    for key, values in updated_order_allocation_combinations.items():
        ...
        updated_order_truck_mapping[(sales_document, sales_document_item, schedule_line, material, po_number)].append((po, period))
        ...
        updated_order_selection_mapping_dict[(sales_document, sales_document_item, material, po_number)][(po, period)].add(schedule_line)
        ...
        if values['shuffle_together_flag'] == 'Y':
            ...
            updated_shuffle_together_mapping_dict[(sales_document, sales_document_item, material, po_number)][(po, period)].add(schedule_line)
            ...
            updated_shuffle_order_schedule_line_mapping_dict[(sales_document, sales_document_item, material, po_number)].add(schedule_line)
...
class ReshufflingStageOneModelConstruction:

    def __init__(self, input_data, optimization_model, raw_output_dict, output_path):
    ...
    def allocation_model_construction(self):
        ...
        # Data Pre Processing
        updated_order_allocation_combinations, updated_truck_selection_combinations, updated_order_truck_mapping, updated_order_selection_mapping_dict, updated_shuffle_together_mapping_dict, updated_shuffle_order_schedule_line_mapping_dict = process_data_preprocessing(self.input_data, order_sel_lst, sel_po_period_lst, sel_period_lst)

        # Model Build and Solve
        logger.info("Build and Solve Optimization Model")
        alloc_model_vars = ReshufflingStageOneDecisionVariable(self.optimization_model, updated_order_allocation_combinations, updated_truck_selection_combinations, selected_order_dict, updated_order_selection_mapping_dict, updated_order_truck_mapping)
        alloc_model_constraints = ReshufflingStageOneConstraints(self.optimization_model, alloc_model_vars, self.input_data, updated_order_allocation_combinations,
                                                       updated_order_truck_mapping, updated_truck_selection_combinations, updated_order_selection_mapping_dict, updated_shuffle_together_mapping_dict, updated_shuffle_order_schedule_line_mapping_dict)
        alloc_model_objectives = ModelObjectives(self.optimization_model, alloc_model_vars, self.input_data, self.output_path, constants.stage_one_reshuffling_objective_type,
                                                constants.stage_one_reshuffling_objectives, constants.stage_one_reshuffling_objective_sense)
        ...
        if not constants.stage_one_reshuffling_manual_hierarchical:
            ...
            self.optimization_model = alloc_model_objectives.create_manual_hierarchical_objectives()
        ...
        if self.optimization_model.status in [gurobipy.GRB.OPTIMAL, gurobipy.GRB.TIME_LIMIT, gurobipy.GRB.INTERRUPTED]:
            ...
            raw_allocation_output_dict['stage_one_opt_run_status'] = 'OPTIMAL'
            ...
            raw_allocation_output_dict['stage_one_opt_run_status'] = 'INFEASIBLE'

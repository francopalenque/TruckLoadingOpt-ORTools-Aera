...
class PostProcessorStageTwo:
    def __init__(self,input_data_stage_two, raw_output_stage_two_dict,output_path, apo_truck_load_fully_utilized, truck_details_fully_utilized, dc, actual_truck_count):
        self.input_data_stage_two = input_data_stage_two
        self.raw_output_stage_two_dict = raw_output_stage_two_dict
        ...
        self.stage_two_output_dict, self.stage_two_proposed_po_count = None,None
        self.actual_lines = len(self.input_data_stage_two.input_data.apo_truck_load_df.drop_duplicates(subset=['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number','requested_delivery_period']))
        self.assign_lines, self.orders_splited = None, None

    def results(self):
        self.stage_two_output_dict, self.stage_two_proposed_po_count = self.post_process_results()



    def post_process_results(self):
        stage_two_output_dict = {}
        ...
        try:
            ...
            kpi_lst = []
            for key, value in self.raw_output_stage_two_dict['kpi_dict'].items():
                kpi_lst.append([self.input_data_stage_two.input_data.plan_id, key,value])
            kpi_solution_df = pd.DataFrame(kpi_lst,columns=["scenario_id", "kpi", "value"])
            stage_two_output_dict['kpi_solution_df'] = kpi_solution_df
        ...
        try:
            ...
            column_seq = ['scenario_id', 'sales_document', 'sales_document_item', 'schedule_line', 'material','po_number', 'dc', 'dc_name',
                          ...
                          'proposed_requested_delivery_date', 'order_allocation', 'flag', 'r_type',
                          'priority_line_flag', 'units_per_pallet', 'weight_per_unit', 'volume_per_unit','base_unit','order_creation_date','order_quantity_unit','planned_goods_issue_date',
                          'order_quantity_in_base_unit','pallet_spot_conversion_factor','materialbycustomer','solver_rejected']

            finial_order_df = pd.DataFrame(columns=column_seq)
            orders_with_no_slots_df = pd.DataFrame(columns=column_seq)
            if len(self.input_data_stage_two.updated_order_allocation_combinations) > 0:
                # Step 1: Convert to DataFrame
                order_allocation_combination_df = pd.DataFrame.from_dict(self.input_data_stage_two.updated_order_allocation_combinations, orient='index')
                # Step 2: Set MultiIndex from the tuple keys
                index_key = ['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number', 'r_type','proposed_po', 'proposed_period']
                ...
                for key, values in self.input_data_stage_two.updated_order_allocation_combinations.items():
                    ...
                    date = self.input_data_stage_two.input_data.period_to_date[period]
                    order_allocation = np.round(self.raw_output_stage_two_dict['order_allocation_dict'][key])

                    order_lst.append([self.input_data_stage_two.input_data.plan_id,sales_document, sales_document_item, schedule_line, material, po_number, r_type, po, period,date,order_allocation])

                order_df = pd.DataFrame(order_lst,columns=['scenario_id', 'sales_document', 'sales_document_item', 'schedule_line', 'material','po_number', 'r_type','proposed_po', 'proposed_period','proposed_requested_delivery_date','order_allocation'])
                finial_order_df = pd.merge(order_allocation_combination_df,order_df,how='left',on=['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number', 'r_type','proposed_po', 'proposed_period'])
                finial_order_df['flag'] = 'SolverResults'
                finial_order_df['actual_pallet_spot'] = finial_order_df['pallet_spot']
                finial_order_df['actual_confirmed_quantity_in_base_unit'] = finial_order_df['confirmed_quantity_in_base_unit']
                finial_order_df['actual_gross_weight'] = finial_order_df['gross_weight']
                finial_order_df['actual_volume'] = finial_order_df['volume']
                finial_order_df['pallet_spot'] = np.where(finial_order_df['r_type'] == 'integer', finial_order_df['order_allocation'],finial_order_df['pallet_spot'])
                finial_order_df['confirmed_quantity_in_base_unit'] = np.where(finial_order_df['r_type'] == 'integer', finial_order_df['pallet_spot'] *  finial_order_df['units_per_pallet'], finial_order_df['confirmed_quantity_in_base_unit'])
                finial_order_df['gross_weight'] = np.where(finial_order_df['r_type'] == 'integer', finial_order_df['pallet_spot'] * finial_order_df['units_per_pallet'] * finial_order_df['weight_per_unit'], finial_order_df['gross_weight'])
                finial_order_df['volume'] = np.where(finial_order_df['r_type'] == 'integer', finial_order_df['pallet_spot'] * finial_order_df['units_per_pallet'] * finial_order_df['volume_per_unit'], finial_order_df['volume'])


            self.assign_lines = len(finial_order_df[finial_order_df['order_allocation'] >= 1].drop_duplicates(subset=['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number','requested_delivery_period']))
            # Group by
            order_po_counts = (finial_order_df[finial_order_df['order_allocation'] >= 1].groupby(['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number',
                 'requested_delivery_period'])['proposed_po'].nunique().reset_index(name='num_trucks_assigned'))
            stage_two_output_dict['order_po_counts'] = order_po_counts
            self.orders_splited = len(order_po_counts[order_po_counts['num_trucks_assigned'] > 1])

            if len(self.input_data_stage_two.updated_order_with_no_slots) > 0:
                # Step 1: Convert to DataFrame
                orders_with_no_slots_df = pd.DataFrame.from_dict(self.input_data_stage_two.updated_order_with_no_slots,orient='index')
                # Step 2: Set MultiIndex from the tuple keys
                ...
                orders_with_no_slots_df['scenario_id'] = self.input_data_stage_two.input_data.plan_id
                ...
                orders_with_no_slots_df['r_type'] = None
            ...
            if len(self.apo_truck_load_fully_utilized) > 0:
                ...
                self.apo_truck_load_fully_utilized['scenario_id'] = self.input_data_stage_two.input_data.plan_id
                self.apo_truck_load_fully_utilized["requested_delivery_period"] = self.apo_truck_load_fully_utilized["requested_delivery_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                self.apo_truck_load_fully_utilized["delivery_period"] =  self.apo_truck_load_fully_utilized["delivery_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                ...
                self.apo_truck_load_fully_utilized['r_type'] = None
            ...
            left_over_apo_truck_load = self.input_data_stage_two.input_data.input_tables_dict['left_over_apo_truck_load']
            if len(left_over_apo_truck_load) > 0:
                left_over_apo_truck_load['proposed_po'] = self.input_data_stage_two.input_data.garbage_truck_po_number
                left_over_apo_truck_load['order_allocation'] = 1
                left_over_apo_truck_load['scenario_id'] = self.input_data_stage_two.input_data.plan_id
                left_over_apo_truck_load["requested_delivery_period"] = left_over_apo_truck_load["requested_delivery_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                # left_over_apo_truck_load["delivery_period"] =  left_over_apo_truck_load["delivery_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                left_over_apo_truck_load["delivery_period"] = None
                left_over_apo_truck_load['proposed_period'] = self.input_data_stage_two.input_data.garbage_truck_period
                left_over_apo_truck_load['proposed_requested_delivery_date'] = self.input_data_stage_two.input_data.period_to_date[self.input_data_stage_two.input_data.garbage_truck_period] if self.input_data_stage_two.input_data.garbage_truck_period is not None else None
                ...
                left_over_apo_truck_load['r_type'] = None
            ...
            # Key: (sales_document, sales_document_item, schedule_line, material, po_number, r_type)
            # Value: >=1 means solver explicitly rejected this order line (extra line, doesn't fit any truck)
            order_non_selection_dict = self.raw_output_stage_two_dict.get('order_non_selection_dict', {})
            if order_non_selection_dict:
                ...
                for key, value in order_non_selection_dict.items():
                    ...
                    non_sel_lst.append([sales_document, sales_document_item, schedule_line, material, po_number, r_type, value])
                non_sel_df = pd.DataFrame(non_sel_lst,columns=['sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number', 'r_type', 'solver_rejected'])
                non_sel_df = non_sel_df[non_sel_df['solver_rejected'] >=1]
                finial_order_df = pd.merge(finial_order_df, non_sel_df, how='left', on=['sales_document', 'sales_document_item', 'schedule_line', 'material','po_number', 'r_type'])
                ...
                non_selected_order_df['proposed_po'] = self.input_data_stage_two.input_data.garbage_truck_po_number
                non_selected_order_df['proposed_period'] = self.input_data_stage_two.input_data.garbage_truck_period
                non_selected_order_df['proposed_requested_delivery_date'] = self.input_data_stage_two.input_data.period_to_date[self.input_data_stage_two.input_data.garbage_truck_period] if self.input_data_stage_two.input_data.garbage_truck_period is not None else None
                non_selected_order_df['pallet_spot'] = np.where(non_selected_order_df['r_type'] == 'integer',non_selected_order_df['solver_rejected'],non_selected_order_df['actual_pallet_spot'])
                non_selected_order_df['confirmed_quantity_in_base_unit'] = np.where(non_selected_order_df['r_type'] == 'integer', non_selected_order_df['pallet_spot'] * non_selected_order_df['units_per_pallet'],non_selected_order_df['actual_confirmed_quantity_in_base_unit'])
                non_selected_order_df['gross_weight'] = np.where(non_selected_order_df['r_type'] == 'integer',non_selected_order_df['pallet_spot'] * non_selected_order_df['units_per_pallet'] * non_selected_order_df['weight_per_unit'],non_selected_order_df['actual_gross_weight'])
                non_selected_order_df['volume'] = np.where(non_selected_order_df['r_type'] == 'integer',non_selected_order_df['pallet_spot'] * non_selected_order_df['units_per_pallet'] * non_selected_order_df['volume_per_unit'],non_selected_order_df['actual_volume'])
            ...
            con_finial_order_df['weight_check_flag'] = con_finial_order_df.apply(lambda row: row['gross_weight'] <= self.input_data_stage_two.input_data.truck_po_level_details.get(row['proposed_po'],{}).get('weight_constraint', 0), axis=1)
            con_finial_order_df['volume_check_flag'] = con_finial_order_df.apply(lambda row: row['volume'] <= self.input_data_stage_two.input_data.truck_po_level_details.get(row['proposed_po'], {}).get('volume_constraint', 0), axis=1)
            con_finial_order_df['pallet_check_flag'] = con_finial_order_df.apply(lambda row: row['pallet_spot'] <= self.input_data_stage_two.input_data.truck_po_level_details.get(row['proposed_po'],{}).get('pallet_constraint', 0), axis=1)
            con_finial_order_df['scenario_name'] = 'stage_two'
            con_finial_order_df = con_finial_order_df[['scenario_id', 'sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number', 'dc',
                 ...
                 'atp_availability_date','delivery_date','original_delivery_date','actual_delivery_date','requested_delivery_period', 'delivery_period', 'requested_delivery_day_name',
                 'delivery_day_name','proposed_po', 'proposed_period', 'proposed_requested_delivery_date', 'order_allocation', 'flag',
                 'r_type', 'priority_line_flag', 'units_per_pallet', 'weight_per_unit', 'volume_per_unit',
                 'delivery_date_check_flag','weight_check_flag', 'volume_check_flag', 'pallet_check_flag','base_unit','order_creation_date','order_quantity_unit','planned_goods_issue_date',
                                                       'order_quantity_in_base_unit','pallet_spot_conversion_factor','materialbycustomer','shuffle_together_flag','scenario_name']]
            stage_two_output_dict['finial_order_df'] = con_finial_order_df
            ...
            column_seq = ['scenario_id', 'sales_document', 'sales_document_item', 'schedule_line', 'material','po_number', 'dc', 'dc_name',
                          ...
                          'delivery_day_name', 'proposed_po', 'proposed_period', 'shuffle_together_flag','proposed_requested_delivery_date', 'order_allocation', 'flag', 'priority_line_flag',
            ...
            finial_order_df = pd.DataFrame(columns=['scenario_id', 'sales_document', 'sales_document_item', 'schedule_line', 'material', 'po_number', 'dc',
                 'dc_name','pallet_spot', 'gross_weight', 'confirmed_quantity_in_base_unit', 'volume', 'requested_delivery_date',
                 'atp_availability_date','delivery_date','original_delivery_date','actual_delivery_date','requested_delivery_period', 'delivery_period', 'requested_delivery_day_name',
                 'delivery_day_name','proposed_po', 'proposed_period', 'proposed_requested_delivery_date', 'order_allocation', 'flag',
                 'r_type', 'priority_line_flag', 'units_per_pallet', 'weight_per_unit', 'volume_per_unit',
                 'delivery_date_check_flag','weight_check_flag', 'volume_check_flag', 'pallet_check_flag', 'base_unit','order_creation_date','order_quantity_unit','planned_goods_issue_date',
                                                    'order_quantity_in_base_unit','pallet_spot_conversion_factor','materialbycustomer','shuffle_together_flag','scenario_name'])
            stage_two_output_dict['finial_order_df'] = finial_order_df

            order_po_counts = pd.DataFrame(columns=['sales_document', 'sales_document_item', 'schedule_line', 'material',
                                                    'po_number', 'requested_delivery_period', 'num_trucks_assigned'])
            stage_two_output_dict['order_po_counts'] = order_po_counts
            left_over_apo_truck_load = pd.DataFrame(columns=column_seq)
        ...
        try:
            ...
            for key, values in self.input_data_stage_two.updated_truck_selection_combinations.items():
                po, period = key
                date = self.input_data_stage_two.input_data.period_to_date[period]
                truck_selection = np.round(self.raw_output_stage_two_dict['truck_selection_dict'][key])
                ...
                if po in self.input_data_stage_two.input_data.truck_po_level_details:
                    original_period = self.input_data_stage_two.input_data.truck_po_level_details[po]['requested_delivery_period']
                    original_date = self.input_data_stage_two.input_data.truck_po_level_details[po]['requested_delivery_date']
                    original_weight_used = self.input_data_stage_two.input_data.truck_po_level_details[po]['total_weight_size_occupied']
                    original_volume_used = self.input_data_stage_two.input_data.truck_po_level_details[po]['total_volume_size_occupied']
                    original_pallet_used = self.input_data_stage_two.input_data.truck_po_level_details[po]['total_pallet_size_occupied']

                weight_used = 0
                for so in values:
                    weight = self.input_data_stage_two.updated_order_allocation_combinations[so]['weight_per_pallet']
                    weight_used += np.round(self.raw_output_stage_two_dict['order_allocation_dict'][so])  * weight
                unused_weight_percent = (weight_limit * truck_selection - weight_used) / weight_limit
                volume_used = 0
                for so in values:
                    volume = self.input_data_stage_two.updated_order_allocation_combinations[so]['volume_per_pallet']
                    volume_used += np.round(self.raw_output_stage_two_dict['order_allocation_dict'][so]) * volume
                unused_volume_percent = (volume_limit * truck_selection - volume_used) / volume_limit
                pallet_used = 0
                for so in values:
                    sales_document, sales_document_item, schedule_line, material, po_number, r_type, po, period = so
                    if r_type == 'integer':
                        pallet_used += np.round(self.raw_output_stage_two_dict['order_allocation_dict'][so])
                    elif r_type == 'decimal':
                        pallet = self.input_data_stage_two.updated_order_allocation_combinations[so]['pallet_spot']
                        pallet_used += np.round(self.raw_output_stage_two_dict['order_allocation_dict'][so]) * pallet
                ...
                truck_lst.append([self.input_data_stage_two.input_data.plan_id, self.dc, po, period, date, original_period, original_date,
                                  truck_selection,weight_limit, volume_limit, pallet_limit, original_weight_used, weight_used,
                                  original_volume_used, volume_used,original_pallet_used, pallet_used, unused_weight, unused_volume, unused_pallet,
                                  unused_weight_percent, unused_volume_percent,unused_pallet_percent])
            ...
            stage_two_proposed_po_count = truck_df['truck_selection'].sum()
            ...
            if len(self.apo_truck_load_fully_utilized) > 0:
                ...
                fully_utilized['scenario_id'] = self.input_data_stage_two.input_data.plan_id
                fully_utilized = fully_utilized[['scenario_id', 'dc', 'po_number', 'period', 'date', 'original_period', 'original_date',
                     'truck_selection','weight_limit', 'volume_limit', 'pallet_limit', 'original_weight_used', 'weight_used',
                     'original_volume_used', 'volume_used','original_pallet_used', 'pallet_used', 'unused_weight', 'unused_volume', 'unused_pallet',
                     'unused_weight_percent','unused_volume_percent', 'unused_pallet_percent', 'flag']]
            ...
            if len(left_over_apo_truck_load) > 0:
                ...
                left_over_truck_capacity_details = self.input_data_stage_two.input_data.input_tables_dict['left_over_truck_capacity_details']
                left_over_truck_capacity_details = left_over_truck_capacity_details.rename(columns={'requested_delivery_date':'original_date'})
                left_over_truck_capacity_details['original_period'] = left_over_truck_capacity_details["original_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                ...
                left_over_po['scenario_id'] = self.input_data_stage_two.input_data.plan_id
            ...
            if len(left_over_apo_truck_load) == 0 and order_non_selection_dict:
                ...
                left_over_truck_capacity_details_non_selected = self.input_data_stage_two.input_data.input_tables_dict['left_over_truck_capacity_details']
                ...
                left_over_truck_capacity_details_non_selected['original_period'] = left_over_truck_capacity_details_non_selected["original_date"].apply(lambda x: self.input_data_stage_two.input_data.date_to_period[x])
                ...
                left_over_po_non_selected['scenario_id'] = self.input_data_stage_two.input_data.plan_id
            ...
            stage_two_output_dict['truck_df'] = con_truck_df

            # Filtered Dataframe
            all_po = list(self.input_data_stage_two.input_data.truck_po_level_details.keys())
            ...
            non_selected_df2['scenario_id'] = self.input_data_stage_two.input_data.plan_id
            non_selected_df2['dc'] = self.dc
            non_selected_df2['action'] = 'Cancelled'
            non_selected_df2['flag'] = 'Not Validate'
            non_selected_df2["original_date"] = non_selected_df2["po_number"].apply(lambda x: self.input_data_stage_two.input_data.truck_po_level_details.get(x, {}).get('requested_delivery_date'))
            sel_non_sel_df = pd.concat([selected_df, non_selected_df, non_selected_df2])
            sel_non_sel_df['scenario_name'] = 'stage_two'
            sel_non_sel_df = sel_non_sel_df[['scenario_id', 'dc', 'po_number', 'period', 'date', 'original_period', 'original_date',
                 'truck_selection', 'weight_limit', 'volume_limit','pallet_limit', 'original_weight_used', 'weight_used', 'original_volume_used', 'volume_used',
                 'original_pallet_used','pallet_used', 'unused_weight', 'unused_volume', 'unused_pallet', 'unused_weight_percent',
                 'unused_volume_percent','unused_pallet_percent', 'flag', 'action', 'scenario_name']]
            stage_two_output_dict['sel_non_sel_df'] = sel_non_sel_df
            ...
            logger.warning(f"Result: {traceback.format_exc()}")
            stage_two_proposed_po_count, stage_two_proposed_under_utilization_range_count = None, None
            truck_df = pd.DataFrame(columns=['scenario_id', 'dc', 'po_number', 'period', 'date', 'original_period', 'original_date',
                         'truck_selection','weight_limit', 'volume_limit', 'pallet_limit', 'original_weight_used', 'weight_used',
                         'original_volume_used', 'volume_used','original_pallet_used', 'pallet_used', 'unused_weight', 'unused_volume', 'unused_pallet',
                         'unused_weight_percent','unused_volume_percent', 'unused_pallet_percent', 'flag'])

            sel_non_sel_df = pd.DataFrame(columns=['scenario_id', 'dc', 'po_number', 'period', 'date', 'original_period', 'original_date',
                 'truck_selection', 'weight_limit', 'volume_limit','pallet_limit', 'original_weight_used', 'weight_used', 'original_volume_used', 'volume_used',
                 'original_pallet_used','pallet_used', 'unused_weight', 'unused_volume', 'unused_pallet', 'unused_weight_percent',
                 'unused_volume_percent','unused_pallet_percent', 'flag', 'action', 'scenario_name'])

            stage_two_output_dict['truck_df'] = truck_df
            stage_two_output_dict['sel_non_sel_df'] = sel_non_sel_df
            logger.warning("Truck Details Solution DataFrame Empty")

        return stage_two_output_dict, stage_two_proposed_po_count
